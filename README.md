...
When `WindowsGraphicsCapture` is active, calibration and detection logs should show `Capture mode: wgc-window` or `wgc-window-region-only`. If you see `foreground-region-only`, the run used `VisiblePixels`, usually because `WindowsGraphicsCapture` failed and explicit fallback was enabled. Minimized windows can still fail depending on Windows/game behavior; in that case the error reports `TargetMinimized` or another WGC-specific reason instead of the old visible-pixel capture-lost message.
