...
The persistent control dock stays visible while switching pages. It shows run state, current audio state, target process, target speakers, OCR engine, OCR backend warm status, selected capture backend, capture status, last OCR text, last detected speaker, and last audio action. Start Guarded Real Audio, Stop, and Restore are available from this dock so they are not hidden inside the Audio page during live play. Start Guarded Real Audio still requires arming the guarded checkbox, passing preflight, a valid OCR region, and an explicit confirmation dialog; raw OCR matches never directly control real audio.
...
The WPF persistent dock and Detection page can select the capture backend for the current run and choose whether backend fallback is allowed. The dock selector and Detection-page selector are synchronized. `Print Current Run Settings` prints the config backend, GUI selected backend, current requested backend, fallback setting, and whether a GUI override is active. Calibration and detection startup first log `GUI selected capture backend: ...`, then backend creation logs `Requested capture backend: ...` and `Actual capture backend: ...`.
